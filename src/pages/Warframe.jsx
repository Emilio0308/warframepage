...
const Warframe = () => {
  ...
  const searchWarframe = (warframeName) => {
    const newListOfWarframes = warframes.filter((warframe) =>
      warframe.name.toLowerCase().includes(warframeName.toLowerCase())
    );
  ...
  useEffect(() => {
    ...
    if (warframeName) {
      const list = warframes.filter((warframe) =>
        warframe.name.toLowerCase().includes(warframeName)
      );
      ...
      <section className="max-w-[1200px] w-full mx-auto p-3 flex flex-col gap-10 pb-[120px]">
        ...
        <form className="flex gap-5 flex-wrap">
          <div className="flex-grow shadow-md relative">
            <input
              placeholder="Search your warframe..."
              onChange={handleClickWriteWarframe}
              value={warframeName}
              className="w-full h-[60px] px-5 outline-none"
              type="text"
            />
            <section className="max-h-[300px] overflow-y-auto w-full bg-gray-300 absolute top-[100%]">
          ...
          <button
            onClick={handleClickSearchWarframe}
            className="h-[60px] text-3xl p-3"
          >
            <FaSearch />
          </button>
          <button
            onClick={handleViewAll}
            className="bg-black text-white p-3 shadow-md shadow-black hover:bg-white hover:text-black"
          >
            VIEW ALL
          </button>
        </form>
